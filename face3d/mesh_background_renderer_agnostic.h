...
class mesh_background_renderer_agnostic
{
public:

  mesh_background_renderer_agnostic(std::vector<IMG_T> const& imgs,
  ...
  template<class CAM_T_OUT>
  void render(CAM_T_OUT const& render_cam_params,
              dlib::array2d<dlib::rgb_pixel> &background,
              face3d::textured_triangle_mesh<TEX_T>& mesh);


  void set_debug_dir(std::string const& debug_dir) { debug_mode_ = true; debug_dir_ = debug_dir; }

private:
  void initialize(std::vector<face3d::textured_triangle_mesh<TEX_T> >& meshes);
  ...
  template <class CAM_T_OUT>
  void apply_coefficients_on_head_mesh(head_mesh const& mesh,
                                       std::vector<sighting_coefficients<CAM_T_OUT> > const& coeffs,
                                       vnl_matrix<double> const & subject_components,
                                       vnl_matrix<double> const & expression_components,
                                       std::vector<face3d::textured_triangle_mesh<TEX_T> >& output);
...
template<class CAM_T, class IMG_T>
template <class CAM_T_OUT>
void mesh_background_renderer_agnostic<CAM_T, IMG_T>::
apply_coefficients_on_head_mesh(head_mesh const& mesh,
                                std::vector<sighting_coefficients<CAM_T_OUT> > const& coeffs,
                                vnl_matrix<double> const & subject_components,
                                vnl_matrix<double> const & expression_components,
                                std::vector<face3d::textured_triangle_mesh<TEX_T> >& output){
    face3d::head_mesh mesh_copy(mesh);
    for (auto const& coeff : coeffs){
      mesh_copy.apply_coefficients(subject_components, expression_components,
                                   coeff.subject_coeffs(), coeff.expression_coeffs());
      output.push_back(face3d::textured_triangle_mesh<TEX_T>(mesh.face_mesh()));
    }
  }


template<class CAM_T, class IMG_T>
mesh_background_renderer_agnostic<CAM_T, IMG_T>::
mesh_background_renderer_agnostic(std::vector<IMG_T> const& imgs,
                                  std::vector<face3d::textured_triangle_mesh<TEX_T> > & subject_meshes,
                                  std::vector<CAM_T> & cam_params,
                                  mesh_renderer &renderer):
  images_(imgs.size()), renderer_(renderer), cam_params_(cam_params),
  UVs_(imgs.size()), Vs_(imgs.size()), Fs_(imgs.size()), visible_mesh_verts_(imgs.size()),
  debug_mode_(false)
{
  const int num_imgs = imgs.size();
  if (cam_params.size() != num_imgs) {
    throw std::logic_error("Different number of images and cameras passed to mesh_background_renderer_agnostic");
  }
  // deep copy images
  for (int i=0; i<images_.size(); ++i) {
    dlib::assign_image(images_[i], imgs[i]);
  }
  std::vector<face3d::textured_triangle_mesh<TEX_T> > meshes;
  initialize(subject_meshes);
}
  ...
  debug_mode_(false)
{
  ...
  for (auto& coef: coeffs)
    this->cam_params_.push_back(coef.camera());
  this->apply_coefficients_on_head_mesh<CAM_T>(mesh, coeffs, subject_components, expression_components, subject_meshes);
  initialize(subject_meshes);
}
...
template<class CAM_T, class IMG_T>
void mesh_background_renderer_agnostic<CAM_T, IMG_T>::
initialize(std::vector<face3d::textured_triangle_mesh<TEX_T> >& meshes)
{
  ...
  for (int i=0; i < num_inputs; ++i) {
    ...
    std::vector<face3d::textured_triangle_mesh<TEX_T> > singleton_vect(1);
    singleton_vect.push_back(meshes[i]);
    renderer_.render(singleton_vect, cam_params_[i], render_img, aux_out);

    auto camera = cam_params_[i].to_camera();
    ...
    meshes[i].get_vertices(mesh_verts_3d);
...
render(sighting_coefficients<CAM_T_OUT> const& render_coeffs,
  ...
  std::vector<CAM_T_OUT> render_cams;
  std::vector<textured_triangle_mesh<TEX_T> > mesh_out;
  std::vector<sighting_coefficients<CAM_T_OUT > > input_coeffs;
  input_coeffs.push_back(render_coeffs);
  this->apply_coefficients_on_head_mesh<CAM_T_OUT>(mesh, input_coeffs, subject_components, expression_components, mesh_out);
  this->render(render_coeffs.camera(), background, mesh_out[0]);
}

template<class CAM_T, class IMG_T>
template<class CAM_T_OUT>
void mesh_background_renderer_agnostic<CAM_T, IMG_T>::
render(CAM_T_OUT const& render_cam_params,
       dlib::array2d<dlib::rgb_pixel> &background,
       face3d::textured_triangle_mesh<TEX_T>& mesh)
{
  if (images_.size() != cam_params_.size()) {
    std::cerr << "ERROR: render(): images_.size() == " << images_.size() << ", cam_params_size() = " << cam_params_.size() << std::endl;
    throw std::logic_error("Different numbers of images and coefficients");
  }
  const int num_input_images = images_.size();

  auto render_camera = render_cam_params.to_camera();
  vgl_rotation_3d<double> render_R = render_cam_params.rotation();

  const int render_nx = render_cam_params.nx();
  const int render_ny = render_cam_params.ny();
  // initialize background
  background.set_size(render_ny, render_nx);
  dlib::assign_all_pixels(background, dlib::rgb_pixel(0,0,0));

  vnl_matrix_fixed<double,3,3> R_render = render_cam_params.rotation().as_matrix();
  ...
  for (int i=0; i<num_input_images; ++i) {
    vnl_matrix_fixed<double,3,3> R_og = cam_params_[i].rotation().as_matrix();
  ...
  std::vector<vgl_point_3d<double> > mesh_verts_3d;
  mesh.get_vertices(mesh_verts_3d);
  ...
  for (int i=0; i<num_input_images; ++i) {
    ...
    //std::cout<<"[face3d::mesh_background_renderer_agnostic:render]: weight "<<image_weights[i]<<std::endl;
