...
public:
  ...
  template<class T>
    estimation_results_t estimate_coefficients(std::vector<std::string> const& img_ids,
                               std::vector<dlib::array2d<vgl_point_3d<float> > > const& semantic_maps,
                               std::vector<T> const& cameras,
                               subject_sighting_coefficients<T> &results);

private:
  template<class T>
    bool estimate_coefficients(std::vector<std::string> const& img_ids,
                               std::vector<std::map<int, vgl_point_2d<double> > > const& vertex_projection_maps,
                               std::vector<T> const& cam_params,
                               std::vector<triangle_mesh> &view_meshes,
                               subject_sighting_coefficients<T> &results);
                      ...
                      subject_sighting_coefficients<T> &results)
{
  ...
  if (semantic_maps.size() != cam_params.size()) {
    throw std::runtime_error("ERROR: different number of semantic maps and camera parameters");
  }
  ...
  retval.success_ =
    estimate_coefficients(img_ids, vertex_projection_maps, cam_params, view_meshes, results);
                      ...
                      subject_sighting_coefficients<T> &results)
{
  ...
  for (int it=0; it<num_its; ++it) {
    ...
    for (int n=0; n<num_images; ++n) {
      ...
      for (auto vp : vertex_projection_maps[n]) {
        vertex_projections.push_back(vp.second);
        vertex_indices.push_back(vp.first);
        mesh_vertices.push_back(view_meshes[n].vertex(vp.first));
      }
      if (debug_mode_) {
    ...
    retval.success_ =
      estimate_coefficients(img_ids, vertex_projection_maps, all_cam_params, view_meshes, results);
                      ...
                      subject_sighting_coefficients<T> &results)
{
  ...
  const int num_images = img_ids.size();
  if (vertex_projection_maps.size() != num_images) {
    throw std::runtime_error("ERROR: Unexpected number of vertex_projection_maps");
  }
  if (cam_params.size() != num_images) {
    throw std::runtime_error("ERROR: Unexpected number of cam_params");
  }
  if (view_meshes.size() != num_images) {
    throw std::runtime_error("ERROR: Unexpected number of view_meshes");
  }
  ...
  for (int n=0; n<num_images; ++n) {
    ...
    auto pcam = cam_params[n].to_camera();
   ...
   **/
  const int num_vars = num_subject_components + num_images*num_expression_components;
  ...
  std::cout << "subject_coeffs = " << subject_coeffs << std::endl;
  // apply newly estimated coefficients to view meshes
  for (int n=0; n<num_images; ++n) {
    head_mesh new_head_mesh(base_mesh_);
    new_head_mesh.apply_coefficients(subject_pca_components_, expression_pca_components_,
                                     subject_coeffs, expression_coeffs[n]);
    view_meshes[n] = new_head_mesh.face_mesh();
  }
  ...
  if (debug_mode_) {
    ...
    for (int s=0; s<num_images; ++s) {
      ...
      view_meshes[s].get_vertices(verts);
